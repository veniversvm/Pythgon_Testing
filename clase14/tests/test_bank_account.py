...
class BankAccountTests2(unittest.TestCase):
    ...
    #  Clase 13
    # En esta version el assertion del bank account es acumulativo
    def test_deposit_multiple_amounts(self):
        test_cases = [
            { "ammount": 100, "expected": 1100},
            { "ammount": 1000, "expected": 2100},
            { "ammount": 4000, "expected": 6100},
        ]
        for case in test_cases:
            with self.subTest(case=test_cases):
                new_balance = self.account.deposit(case["ammount"])
                self.assertEqual(new_balance, case["expected"])
    # En esta version el regeneramos el Bank Account
    def test_deposit_multiple_amounts2(self):
        test_cases = [
            { "ammount": 100, "expected": 1100},
            { "ammount": 1000, "expected": 2000},
            { "ammount": 4000, "expected": 5000},
        ]
        for case in test_cases:
            with self.subTest(case=case):
                self.account = BankAccount(balance=1000, log_file="multiple_transactions")
                new_balance = self.account.deposit(case["ammount"])
                self.assertEqual(new_balance, case["expected"])
